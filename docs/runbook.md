...
| Implementation transition fails: `"Transition is allowed only if the issue has sub-task"` | The *Start implementation* validator requires a sub-task and none exists | Handled by `Create_Approval_Subtask` (runs before `Walk_to_Planning`). If it recurs, check `Find_Assignee_Account`/`Create_Approval_Subtask` succeeded — most often `SubtaskIssueTypeName` isn't a valid sub-task type in the project. |
| Sub-task assigned to a **random colleague** | `Find_Assignee_Account` used the Jira **Cloud** param `query=`; on Server/DC that is ignored and the search returns the *first assignable user* | Fixed: the search now uses the Server/DC param `username=` (`GET user/assignable/search?issueKey={Clone_Key}&username={SubtaskAssigneeName}`), so the result is actually filtered. |
| Run fails `AssigneeNotFound` | `Find_Assignee_Account` (now filtered) matched no user — `SubtaskAssigneeName` resolves to nobody, or `sentinelsvc` lacks *Browse users* | `Assert_Assignee_Found` terminates the run on an empty result (instead of mis-assigning). Confirm `SubtaskAssigneeName` matches a real user by username/display name/email, and that `sentinelsvc` can browse users. If the email isn't the user's searchable Jira email, set `SubtaskAssigneeName` to their username or display name. |
...
| `SubtaskAssigneeName` | `lsyh.secops@lhsystems.com` | Assignee for the sub-task, given as a search query (username, display name, or email). `Find_Assignee_Account` resolves it to the actual Jira login at runtime; if it matches nobody, `Create_Approval_Subtask` fails and stops the run. |
