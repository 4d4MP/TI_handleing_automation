...
| `Create_Approval_Subtask` fails: `assignee: User '…' does not exist` | Jira sets `assignee` by login `name`, not email/display name; `Find_Assignee_Account` returned no match | `Find_Assignee_Account` (`GET user/assignable/search?issueKey={Clone_Key}&query={SubtaskAssigneeName}`) resolves the login from the query, so a bad/empty result means the query matched nobody **or** the service account lacks *Browse users*. Confirm `SubtaskAssigneeName` matches a real user (username/display/email) and that `sentinelsvc` can browse users. |
