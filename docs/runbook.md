...
| Sub-task stays **Open** on a **successful** run; `Approve_Subtask_Post` returns `400 BadRequest` with `errors.resolution = "The selected resolution cannot be chosen during this action."` | The `Resolved` transition screen has a Resolution field whose default value isn't in the transition's allowed resolutions, so a bare transition POST is rejected | **Resolved:** the success POST now sets a resolution explicitly, chosen from that transition's `fields.resolution.allowedValues` (fetched via `expand=transitions.fields`) — preferring `SubtaskSuccessResolutionName` (default `Successful`), else the first allowed value. If it recurs, `Successful` may not be offered and the first allowed value is wrong for the desired outcome — set `SubtaskSuccessResolutionName` to a resolution that appears in the `Resolve issue` transition's allowed list (e.g. `Done`). |
